...
object ChildActorsExercisesWithContextBecome extends App {
  ...
  class WordCounterMaster extends Actor {
    ...
    override def receive = {
      case Initialize(nChild) =>
        val freeWorkers = (1 to nChild).map(n => context.actorOf(Props[WordCounterWorker], s"wcw$n"))
        println(s"${freeWorkers.length} workers initialized")
        context.become(withChildrenWorkers(freeWorkers, Seq()))
    }

    def withChildrenWorkers(freeWorkers: Seq[ActorRef], runningWorkers: Seq[ActorRef]) : Receive = {
      case Text(str) => {
        if(freeWorkers.nonEmpty) {
          val firstFree = freeWorkers.take(1)
          println(s"firstFree $firstFree")
          val updatedRunningWorkers = runningWorkers ++ firstFree
          val updatedFreeWorkers = freeWorkers.filterNot(_ == firstFree.head)
          println(s"freeWorkers now ${updatedFreeWorkers.map(_.path)}")
          println(s"runningWorkers now ${updatedRunningWorkers.map(_.path)}")
          firstFree.head ! WordCountTask(firstFree.head, str)
          context.become(withChildrenWorkers(updatedFreeWorkers, updatedRunningWorkers))
        } else
          self forward  Text(str) //infinite loop in the hope that sometime in future one of the workers will be free
      }
      ...
      case WordCountReply(workerRef, count) => {
        val updatedRunningWorkers = runningWorkers.filterNot(_ == workerRef)
        println(s"runningWorkers updated ${updatedRunningWorkers.map(_.path)}")
        val updatedFreeWorkers = freeWorkers ++ Seq(workerRef)
        println(s"freeWorkers updated ${updatedRunningWorkers.map(_.path)}")
